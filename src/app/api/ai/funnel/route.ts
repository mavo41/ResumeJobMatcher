...
export async function GET(request: Request) {
  try {
    ...
    // Use the existing Convex query
    const funnelData = await convex.query(api.ai.getHiringFunnel, {
      userId: employerId,
    ...
    const conversionRates = {
      appliedToInterview: funnelData.applied > 0
        ? Math.round((funnelData.interviewing / funnelData.applied) * 100)
        : 0,
      interviewToOffer: funnelData.interviewing > 0
        ? Math.round((funnelData.offer / funnelData.interviewing) * 100)
        : 0,
      offerToHired: funnelData.offer > 0
        ? Math.round((funnelData.hired / funnelData.offer) * 100)
        : 0,
      overall: total > 0
        ? Math.round((funnelData.hired / funnelData.sourced) * 100)
        : 0,
    };
...
function generateFunnelInsights(funnelData: any, conversionRates: any): string[] {
  ...
  if (applied > 0 && interviewed === 0) {
    insights.push("No candidates are progressing to interviews. Consider reviewing your screening criteria.");
  }

  if (interviewed > 0 && offered === 0) {
    insights.push("Candidates are interviewing but not receiving offers. Consider reviewing your interview process.");
  }

  if (offered > 0 && hired === 0) {
    insights.push("Offers are being extended but not accepted. Consider reviewing your offer competitiveness.");
  }

  if (conversionRates.appliedToInterview < 20 && applied > 10) {
    insights.push("Low application-to-interview conversion rate. Consider broadening your screening criteria.");
  }
  ...
  if (insights.length === 0) {
    if (hired > 0) {
      insights.push(`Successfully hired ${hired} candidates. Overall conversion rate: ${conversionRates.overall}%.`);
    } else {
